...
export function LoanDetailSheet({ open, onOpenChange, loan, canEdit, onReturn }: Props) {
  ...
  if (!loan) {
    return (
      <EntitySheet open={open} onOpenChange={onOpenChange}>
        <EntitySheetHeader>
          <EntitySheetTitle>Préstamo</EntitySheetTitle>
        </EntitySheetHeader>
        <EntitySheetBody />
      </EntitySheet>
    );
  ...
  return (
    <EntitySheet open={open} onOpenChange={onOpenChange}>
      <EntitySheetHeader>
        <EntitySheetTitle>{loan.item?.name ?? "Artículo"}</EntitySheetTitle>
        <EntitySheetDescription>
          {loan.quantity} {loan.item?.unit ?? "piezas"} prestadas a {borrowerName}
        </EntitySheetDescription>
      </EntitySheetHeader>
      ...
      <EntitySheetFooter>
        ...
        {canEdit && pending > 0 ? (
          <Button type="button" onClick={() => onReturn(loan)}>
